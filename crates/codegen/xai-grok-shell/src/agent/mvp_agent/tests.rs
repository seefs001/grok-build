...
#[tokio::test(flavor = "current_thread")]
async fn acp_set_config_option_resolves_effort_id_to_live_reasoning_effort() {
    use crate::agent::config::{EndpointsConfig, ModelEntry, ModelVariant};
            ...
            let actor = tokio::task::spawn_local(async move {
                while let Some(command) = cmd_rx.recv().await {
                    match command {
                        ...
                        // Upstream applies an effort change on the live model without a
                        // model switch; the selector id ("deep") must resolve to its level.
                        TestSessionCommand::SetReasoningEffort { effort, responds_to } => {
                            assert_eq!(effort, ReasoningEffort::Xhigh);
                            let _ = responds_to.send(Ok(acp::ModelId::new("routed-high")));
                            break;
                        }
