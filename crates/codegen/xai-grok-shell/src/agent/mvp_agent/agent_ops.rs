...
impl MvpAgent {
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        config.fast = fast;
        // Fork: `[models].reasoning_summary` is a global fallback; upstream's
        // per-model `[model.<id>].reasoning_summary` wins when set.
        config.reasoning_summary = config.reasoning_summary.or(reasoning_summary);
        config
    }
