...
fn resolve_model_override_to_config(
    ...
    // Per-model `reasoning_summary` on the override wins; the parent's value is only a fallback.
    config.reasoning_summary = config
        .reasoning_summary
        .or(ctx.sampling_config.reasoning_summary);
